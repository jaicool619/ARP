...
return type = int
parameter = none
This function will signup new users
*/

int signUp()
{
    struct user *user_pointer = NULL;
    FILE *file_pointer = NULL;
    user_pointer = (struct user *)malloc(sizeof(struct user)); // initializing memory to structure pointer user_pointer:
    system("clear");
    ...
    free(user_pointer); // freeing the  memory of user_pointer:
...
int login()
{
    ...
    char log_user[SIZE], log_pass[SIZE]; // initialize character array:
    int flag = FALSE;                    // FLag variable to store the returning value and check for TRUE and FALSE
    ...
    else
    {
        printf("\n\t\t\tlogged in\n\n");
        sleep(SECONDS);
        return intoAcc(log_pass); // redirected to main account:
...
This function will check username is already present or not
*/

int check_details(char *user_name)
{
    int flag = FALSE;
...
This function to search username and password and return TRUE or FALSE
*/
...
return type = int
parameters = none
This function to to display menu driven options to the users
*/
int userlogin()
{

    int choice; // variable declaration to enter the user choices:
